...
The scored units: a signal, and the chunk / batch / file / corpus verdicts
built from lists of them. The vocabulary those verdicts are expressed in
(:mod:`complexity.tiers`) and the between-file dependency graph
(:mod:`complexity.dependencies`) are separate modules, re-exported here so
``from complexity.models import ...`` keeps reaching everything.
...
from .dependencies import DependencyGraph as DependencyGraph
from .tiers import ComplexityTier as ComplexityTier
from .tiers import TranslationParity as TranslationParity
from .tiers import TShirtSize as TShirtSize
from .tiers import max_size as max_size
from .tiers import max_tier as max_tier
from .tiers import parity_rank as parity_rank
from .tiers import size_rank as size_rank
from .tiers import tier_rank as tier_rank
from .tiers import worst_parity as worst_parity
...
class ChunkComplexity(_ComplexityBase):
    ...
    start_line: int = 0
    end_line: int = 0
    # The datasets this chunk reads and writes, carried up from the chunker's
    # metadata so a report can state a file's data interface without the reader
    # going back to the SAS. Unlike ``text`` (see complexity/report.py), these
...
class FileComplexity(_ComplexityBase):
    ...
    chunk_count: int = 0
    line_count: int = 0
    # COMMENT_BLOCK chunks the analysis excluded. Reported so this file's
    # `chunk_count` can be reconciled against the chunker's own count — a gap
...
class CorpusComplexityReport(BaseModel):
    ...
    # Corpus-level dependency structure — which file has to be migrated before
    # which. None when cross-file analysis was switched off, since there is
    # then nothing to have resolved edges from.
    ...
    def to_markdown(self, *, top: int = 10, graph_image: str | None = None) -> str:
        ...
        *graph_image* is a path to a rendered dependency-graph PNG, relative to
        ...
        the edge table. Omitted on the stdout path, which has nowhere to put an
        ...
        if self.graph is not None:
            # Imported here, not at module scope: this module is the pure-data
            # layer and imports nothing else in the package (see its docstring).
            # complexity.graph imports *these* models, so a top-level import
            # would be a cycle.
