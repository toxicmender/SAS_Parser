...
from ..tokens import TokenBudgetPolicy
...
def _merge_singletons_into_batch(
    ...
    token-budgeted packing (:func:`coalesce_into_batches` with a shared
    :class:`TokenBudgetPolicy`), where flattened members of adjacent items get the
...
def coalesce_into_batches(
    ...
    *,
    ...
    With ``policy=None`` (default), real dependency batches pass through
    ...
    *max_chunks* members — fewer, larger requests — while preserving corpus
    ...
    With a token budget policy, packing is **token-budgeted and extends to
    ...
    (the policy's available input tokens, or *max_chunks* total member
    ...
    if policy is not None:
        return _pack_into_batches(
            ...
            max_tokens=policy.available_input_tokens,
    ...
    **global-context batch**, emitted first with ``is_global_context=True``.
    ...
    def from_files(
        ...
        **batcher_kwargs
        ...
        """
        from .chunking import SasSemanticChunker  # avoid circular at module level
