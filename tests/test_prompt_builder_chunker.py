...
def test_single_section_becomes_one_chunk_with_breadcrumb():
    ...
    chunks = InstructionChunker(min_tokens=120, max_tokens=900).chunk(sections)
    assert len(chunks) == 1
    chunk = chunks[0]
...
def test_role_is_propagated():
    ...
    chunks = InstructionChunker(min_tokens=10).chunk(
...
def test_small_siblings_under_same_parent_merge():
    ...
    # Each _words(40) section is ~80 tokens, so the merge threshold has to sit
    # above all three combined for them to land in one chunk.
    chunks = InstructionChunker(min_tokens=250, max_tokens=900).chunk(sections)
    assert len(chunks) == 1
    chunk = chunks[0]
...
def test_different_parent_breaks_merge():
    ...
    chunks = InstructionChunker(min_tokens=100).chunk(sections)
    assert len(chunks) == 2
...
def test_oversized_section_splits_into_windows():
    ...
    chunks = InstructionChunker(min_tokens=120, max_tokens=300).chunk(sections)
    assert len(chunks) > 1
...
def test_split_windows_are_uniquely_ided():
    ...
    sections = [_section("Guide > Long", big)]
    chunks = InstructionChunker(max_tokens=150).chunk(sections)
    ids = [c.chunk_id for c in chunks]
    assert len(ids) == len(set(ids))


def test_split_overlapping_carries_overlap_between_windows():
    paras = [f"para{i} {_words(4)}" for i in range(4)]  # ~10 tokens each
    text = "\n\n".join(paras)
    # Two paragraphs to a window, and an overlap big enough to carry one back.
    windows = _split_overlapping(text, max_tokens=25, overlap_tokens=12)
...
def test_giant_single_paragraph_is_hard_split():
    ...
    chunks = InstructionChunker(max_tokens=100).chunk(sections)
    assert len(chunks) >= 5
    for chunk in chunks:
        body = chunk.text.split("\n\n", 1)[1]
