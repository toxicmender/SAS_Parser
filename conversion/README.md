...
4. `upload.py` writes converted notebooks/scripts, effective-prompt Markdown
   artifacts under each run's `prompts/` subdirectory, and validation
   artifacts.
...
| File | Responsibility |
|---|---|
| `paths.py` | Source, converted-output, prompt, and validation folder conventions. |
| `requests.py` | Request/list row models, projections, and status updates. |
| `sources.py` | Source discovery and reading. |
| `upload.py` | Deliverable, prompt-artifact, and validation uploads plus notebook rendering. |
